...
# 4.6 m. For each αp in the sweep, runs the time integration to
# t = 17T, extracts κ and η probe time series, and saves the full
# free-surface and beam-deflection profiles to JLD2.
#
...
function make_plots(ts, κ_profiles, κ_max_t, η_max_t,
    ...
    savefig(plt_max, joinpath(plot_dir, "case2_maxamp_ap$(ap_str).png"))
    ...
    savefig(plt_eta, joinpath(plot_dir, "case2_beam_ap$(ap_str).png"))
    ...
    savefig(plt_env, joinpath(plot_dir, "case2_envelope_ap$(ap_str).png"))

    # Snapshot profiles at 13T, 14T, 15T
    i_ss = findall(tᵢ -> win_start*T <= tᵢ <= win_end*T, ts)
    ...
    savefig(plt_fs, joinpath(plot_dir, "case2_profiles_ap$(ap_str).png"))
    ...
    savefig(plt_ts, joinpath(plot_dir, "case2_timeseries_ap$(ap_str).png"))
end
...
let
    order = 2
    model_wm  = Thesis_FPVBarrier.build_model("meshes/case2_length_sweep/case2_L_4.msh")
...
function run_case2_sweep_L4()
    # -- Settings -------------------------------------------------------------------
    order      = 2
    dt_per_T   = 40        # timesteps per period
    tend_T     = 17        # total run duration [T] — clean window [12T,15T] + buffer
    win_start  = 12.0      # extraction window start [T]
    win_end    = 15.0      # extraction window end [T]
    ...
    plot_dir    = "results/plots/case2_timedomain/sweep/L4"
    vtk_base    = "results/vtk/case2_sweep_L4"
    jld2_dir    = "results/jld2/case2/L4"
    mkpath(plot_dir)
    mkpath(jld2_dir)
    ...
    for αp in αp_sweep
        ...
        model2  = Thesis_FPVBarrier.build_model("meshes/case2_length_sweep/case2_L_4.msh")
        ...
        # Probe setup — no damping zones, probes placed close to beam
        λ    = 2π / k
        ...
        # -- Spatial DOF coordinates for η (beam) ------------------------------------------
        ...
        # Single-stage solver: θ=0.5 from t=0
        step = 0
        ...
        @time for ((uh, ph, κh, ηh, ζh), t) in xht
            ...
            # Store probe values and diagnostics every step
            ...
            # VTK: only in extraction window, every vtk_every steps
        ...
        # -- Save JLD2 for plot recreation ---------------------------------------------------------
        jld2_path = joinpath(jld2_dir, "case2_L4_ap$(ap_str).jld2")
        ...
        # downstream probe signal with a standing wave, making the decomposition
        # unreliable (see methodology, Chapter 4). Results are reported qualitatively
        # instead, via the κ/η snapshot profiles saved above.
